...
\begin{itemize}
	\item Standard inventory rule with stochastic demand (see e.g. \cite{Nahmias_2008}): The firms compute different delivery volumes for all served malls.
...
\item Determination of the (estimated) demand in all malls over the selling period.   
\begin{itemize}
	\item Used for the determination of the production quantity in the following periods.
\end{itemize}
		\[
...
\item With a probability $prob^{Inno}$ the R\&D is successful.
\begin{itemize}
...
\item Once a week a household goes to the closest located mall.
