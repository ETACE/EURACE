...
//	double_array * residual_var;
//	double_array * var_per_installment;
...
//	bankruptcy_state = 0;
//	total_resources = 0.0;
//	residual_var = init_double_array();
//	var_per_installment = init_double_array();
...
in_Household_agent = 0;
				...
				if(strcmp(agentname, "Firm") == 0)
				{
					...
					if( flag == 0 )
					{
						//add_Firm_agent(id, interest, credit_offer, contacted_bank, total_debt, asset, credit_demand, installment_amount, installment_number, loans_number, outstanding_debt, lending_bank_id, interest_amount, bankruptcy_state, total_resources, residual_var, var_per_installment, value_at_risk, interest_left, current_equity, current_debt);
						add_Firm_agent_internal(current_Firm_agent, Firm_01_state);
				...
				else if(strcmp(agentname, "Bank") == 0)
				{
					...
					if( flag == 0 )
					{
						//add_Bank_agent(id, cash, total_credit, equity, bce_debt, alfa, var, min_interest, gamma, profits, lambda, bce_interest, debt_period);
						add_Bank_agent_internal(current_Bank_agent, Bank_01_state);
				...
				else if(strcmp(agentname, "Household") == 0)
				{
					...
					if( flag == 0 )
					{
						...
						add_Household_agent_internal(current_Household_agent, Household_01_state);
...
//				bank_account = 0;
			...
			if(strcmp(buffer, "var_per_installment") == 0) in_var_per_installment = 1;
			if(strcmp(buffer, "/var_per_installment") == 0) in_var_per_installment = 0;
			...
			if(strcmp(buffer, "var") == 0) in_var = 1;
			if(strcmp(buffer, "/var") == 0) in_var = 0;
		...
		else if(c == '<')
		{
			...
			else if(in_Firm_agent == 1)
			{
				...
				if(in_residual_var) read_double_dynamic_array(buffer, &j, &current_Firm_agent->residual_var);
				if(in_var_per_installment) read_double_dynamic_array(buffer, &j, &current_Firm_agent->var_per_installment);
			...
			else if(in_Bank_agent == 1)
			{
				...
				if(in_var) current_Bank_agent->var = atof(buffer);
...
//////	free_double_array(residual_var);
//	free_double_array(var_per_installment);
...
void write_Firm_agent(FILE *file, xmachine_memory_Firm * current)
{
	...
	write_double_dynamic_array(file, &current->residual_var);
	fputs("</residual_var>\n", file);
		fputs("<var_per_installment>", file);
	write_double_dynamic_array(file, &current->var_per_installment);
	fputs("</var_per_installment>\n", file);
