...
void initialise_pointers()
{
	...
	//temp_xmachine_Bank_06 = NULL;
	Bank_06_state = init_Bank_state();//&temp_xmachine_Bank_06;
...
xmachine_memory_Firm * init_Firm_agent()
{
	...
	init_double_array(&current->residual_var);
	init_double_array(&current->var_per_installment);
...
void free_Firm_agent(xmachine_memory_Firm_holder * tmp, xmachine_memory_Firm_state * state)
{
	...
	free_double_array(&tmp->agent->residual_var);
	free_double_array(&tmp->agent->var_per_installment);
 ...
 * \param var_per_installment Variable for the X-machine memory.
...
void add_Firm_agent(int id, double_array * interest, double_array * credit_offer, int_array * contacted_bank, double total_debt, double asset, double credit_demand, double_array * installment_amount, int installment_number, int loans_number, double_array * outstanding_debt, int_array * lending_bank_id, double_array * interest_amount, int bankruptcy_state, double total_resources, double_array * residual_var, double_array * var_per_installment, double_array * value_at_risk, double_array * interest_left, double current_equity, double current_debt)
{
	...
	copy_double_array(residual_var, &current->residual_var);
	copy_double_array(var_per_installment, &current->var_per_installment);
...
/*	while(*Bank_06_state)
	{
		free_Bank_agent(*Bank_06_state, Bank_06_state);
	}
	while(*Bank_05_state)
	{
...
/** \fn void add_Bank_agent(int id, double cash, double total_credit, double equity, double bce_debt, double alfa, double var, double min_interest, double gamma[], double profits[], double lambda, double bce_interest, double debt_period)
 ...
 * \param var Variable for the X-machine memory.
...
{
/*	while(*Household_05_state)
	{
		free_Household_agent(*Household_05_state, Household_05_state);
	}
	while(*Household_04_state)
	{
		free_Household_agent(*Household_04_state, Household_04_state);
	}
...
/** \fn double_array get_residual_var()
...
/** \fn double_array get_var_per_installment()
 * \brief Get var_per_installment memory variable from current X-machine.
 * \return Value for variable.
 */
double_array * get_var_per_installment()
{
	if(current_xmachine->xmachine_Firm) return &(*current_xmachine->xmachine_Firm).var_per_installment;
...
/** \fn void set_var(double var) 
 * \brief Set var memory variable for current X-machine.
 * \param var New value for variable.
 */
void set_var(double var)
{
	if(current_xmachine->xmachine_Bank) (*current_xmachine->xmachine_Bank).var = var;
}

/** \fn double get_var()
 * \brief Get var memory variable from current X-machine.
 * \return Value for variable.
 */
double get_var()
{
	if(current_xmachine->xmachine_Bank) return (*current_xmachine->xmachine_Bank).var;
