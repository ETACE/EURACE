...
int main(int argc, char * argv[])
{
    ...
    pSuite = CU_add_suite("Suite_Household_Financial_Market_unittests", init_suite1, clean_suite1);
    if (NULL == pSuite)
    {
        CU_cleanup_registry();
        return CU_get_error();
    ...
    if(CU_add_test(pSuite, "Household_select_strategy", unittest_Household_select_strategy) == NULL)
    {
        CU_cleanup_registry();
        return CU_get_error();
    }
