...
int Firm_compute_dividends()
{
   /*1: print the printf-statements*/
	...
	//Compute the dividends per share
	 CURRENT_DIVIDEND_PER_SHARE = TOTAL_DIVIDEND_PAYMENT/CURRENT_SHARES_OUTSTANDING;
...
int Firm_compute_balance_sheet()
{
    ...
    TOTAL_ASSETS = PAYMENT_ACCOUNT + TOTAL_VALUE_CAPITAL_STOCK + TOTAL_VALUE_LOCAL_INVENTORY+ACCRUALS +ACCRUALS_PAY_OUT ;
        
    PLANNED_TOTAL_ASSETS = PAYMENT_ACCOUNT  + PLANNED_VALUE_CAPITAL_STOCK +  TOTAL_VALUE_LOCAL_INVENTORY+ACCRUALS+ACCRUALS_PAY_OUT ;
     ...
     for (i=0; i<imax;i++)
     {
       ...
       //step 3: actual dividend payments
        ...
        if(TOTAL_DIVIDEND_PAYMENT>0.0)
        {
            ...
            ACCRUALS_PAY_OUT -= TOTAL_DIVIDEND_PAYMENT;
        }   
