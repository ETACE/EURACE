...
printf("eurostat_send_specific_skills_message->productivity_progress %f \n",eurostat_send_specific_skills_message->productivity_progress);
					...
					{
						if(eurostat_send_specific_skills_message->productivity_progress > 0.0)
						{
							temp_progress = eurostat_send_specific_skills_message->productivity_progress*0.5;
						}
						else
						{
							temp_progress = eurostat_send_specific_skills_message->productivity_progress;
						}
						
						WAGE_OFFER = WAGE_OFFER*(1+temp_progress);

						int i;
						for(i = 0; i< EMPLOYEES.size; i++)
						{

							EMPLOYEES.array[i].wage = EMPLOYEES.array[i].wage*(1+ temp_progress);
