...
void freeGovernment(Government * head)
{
	/* Tempory element needed for loop */
	Government * tmp;
...
Government * addGovernment(Government ** pointer_to_Government, Government * current)
{
	/* The new tail of the linked list */
	Government * tail;
...
*****************************************************************/
int getiteration(char * filepath, int itno, firm ** pointer_to_firms, household ** pointer_to_households,IGfirm ** pointer_to_IGfirms,mall ** pointer_to_malls,Eurostat ** pointer_to_Eurostats, Government ** pointer_to_Government)
{
	/* Pointer to file */
	FILE *file;
	...
	int intotal_sold_quantity, inplanned_output, inaccount, inprice_last_month, inmean_specific_skills, inmean_wage, intotal_debt, inreal_exports, innominal_exports, incapital_costs, inactual_cap_price;
	/*Household*/
	int  inwage, inwage_reservation, ingeneral_skill, inspecific_skill, inemployee_firm_id;
	int inemployer_region_id,injust_employed, injust_unemployed;
	...
	tail_Government = *pointer_to_Government;
			...
			current_Eurostat ->gdp = gdp;
				...
				if(strcmp(name, "Government") == 0)
				{
					...
					if(tail_Government == NULL)
					{
						//printf("tail is null allocate more memory\n");
						/* Allocate memory */
						tail_Government = addGovernment(pointer_to_Government, current_Government);
					}
					//else printf("tail exisits\n");
...
if(inagent && inplanned_output)  { planned_output  = atof(buffer); }
	 ...
	 if(inagent && inprice_index)  { price_index  = atof(buffer); }	
		 ...
		 if(inagent && ingdp)  {gdp  = atoi(buffer); }
			/* Reset buffer */
			i = 0;
		}
	...
	if(tail_Government)
	{
		freeGovernment(tail_Government);
		/* Make pointer to tail equal NULL */
...
void savedatatofile(int itno, firm ** pointer_to_firms, household ** pointer_to_households,IGfirm ** pointer_to_IGfirms,mall ** pointer_to_malls, Eurostat ** pointer_to_Eurostats, Government ** pointer_to_Government)
{
	FILE *file;
	char data[100];
	firm * current_firm;
	...
	current_Government = *pointer_to_Government;
...
current_Government = *pointer_to_Government;
	...
	while(current_Government)
	{
	    ...
	    total_debt_government =  current_Government->total_debt;
...
/*----------------------------HOUSEHOLDS------MONTHLY DATA---------------------------------------*/
	...
	if(itno%20 == 0)
	{
		...
		file = fopen("data-government.csv", "a");
			...
			//2
			sprintf(data, "%f", total_debt_government/gdp);
			fputs(data, file);
			fputs("\n", file);
			fclose(file);
...
/*----------------------------FIRM MONTHLY AND REGIONAL DATA---------------------------------------*/
	...
	double consumption_nominal_total = consumption_nominal_reg1+consumption_nominal_reg2;

	double nominal_investments_total= nominal_investments_reg1+nominal_investments_reg2;
	
	
	double nominal_change_in_mall_inventory_total = nominal_change_in_mall_inventory_reg1+nominal_change_in_mall_inventory_reg2;
...
;
	...
	file = fopen("data-government.csv", "w");
	fclose(file);
	...
	*argv++;
	...
	while(stilldata)
	{
		stilldata = getiteration(filepath, iteration_number, p_firms, p_household,p_IGfirms, p_malls, p_Eurostats,p_Governments );
		
		if(stilldata) savedatatofile(iteration_number, p_firms, p_household,p_IGfirms, p_malls,p_Eurostats,p_Governments);
		//iteration_number++;
		iteration_number+=20;
