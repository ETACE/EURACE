...
int getiteration(char * filepath, int itno, firm ** pointer_to_firms, household ** pointer_to_households, IGfirm ** pointer_to_IGfirms, mall ** pointer_to_malls, Eurostat ** pointer_to_Eurostats)
{
	...
	int inaverage_debt_earnings_ratio, inaverage_debt_equity_ratio, inwage_profit_ratio;
	...
	double average_debt_earnings_ratio, average_debt_equity_ratio, wage_profit_ratio;
	...
	while(reading == 1)
	{
		...
		if(c == '>')
		{
			...
			buffer[i] = 0;
			if(strcmp(buffer, "states") == 0) { iteration = 1; }
			...
			if(strcmp(name, "Eurostat") == 0)
				{
					...
					current_Eurostat->region_id 				 = region_id;
				    ...
				    current_Eurostat->wage_profit_ratio          = wage_profit_ratio;
	...
	if(strcmp(buffer, "/planned_production_quantity") == 0) { inplanned_production_quantity = 0; }
			...
			if(strcmp(buffer, "wage_profit_ratio") == 0) { inwage_profit_ratio = 1; }
		       if(strcmp(buffer, "/wage_profit_ratio") == 0) { inwage_profit_ratio= 0; }	
		...
		else if(c == '<')
		{
			...
			if(inagent && inwage_profit_ratio)  { wage_profit_ratio  = atof(buffer); }
...
/*----------------------------FIRM MONTHLY AND REGIONAL DATA---------------------------------------*/
	...
	fputs(data, file);
	fputs("\t", file);
	sprintf(data, "%f", wage_profit_ratio);
	fputs(data, file);
	fputs("\n", file);
