 ...
 */
int IGFirm_read_loan_acceptance()
{
  ...
  int nr_periods_before_maturity;
	...
	START_LOAN_ACCEPTANCE_MESSAGE_LOOP
		...
		if(TOTAL_EXTERNAL_FINANCING_OBTAINED > 0.0)
		{
            ...
            add_debt_item(&LOANS, bank_id, credit_id, loan_value, interest_rate, interest_payment, 	
		debt_installment_payment, nr_periods_before_maturity);            
		}
	...
	//Check if obtained financing is sufficent to satisfy financial obligations
		...
		if(PAYMENT_ACCOUNT - FINANCIAL_LIQUIDITY_NEEDS<0.0)
		{
			...
			if(PAYMENT_ACCOUNT<(TOTAL_INTEREST_PAYMENT + TOTAL_DEBT_INSTALLMENT_PAYMENT 
			+ TAX_PAYMENT))
			{
		...
		}else
		{
			...
			FINANCIAL_RESOURCES_FOR_PRODUCTION = PAYMENT_ACCOUNT 
