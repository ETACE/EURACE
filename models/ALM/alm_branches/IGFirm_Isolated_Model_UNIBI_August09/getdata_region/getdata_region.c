		...
		if(c == '>')
		{
			...
			if(strcmp(name, "IGFirm") == 0)
				...
				current_IGfirm->capital_good_store_after_production = capital_good_store_after_production;
					...
					current_IGfirm->wage_offer_for_rd = wage_offer_for_rd;
			...
			if(strcmp(buffer, "wage_offer_for_rd") == 0) { inwage_offer_for_rd = 1; }
			if(strcmp(buffer, "/wage_offer_for_rd") == 0) { inwage_offer_for_rd = 0; }
...
if(strcmp(buffer, "monthly_tax_revenues") == 0) { inmonthly_tax_revenues = 1; }
		...
		else if(c == '<')
		{
			...
			if(inagent && inwage_offer_for_rd)  { wage_offer_for_rd = atof(buffer); }
...
//Regions: wageoffer
	...
	for(i = 0; i <= no_regions; i++)
	{
		...
		fputs(data, file);
		fputs("\t", file);
		sprintf(data, "%f", Data_IGFirm[i].wage_offer_for_rd);
		fputs(data, file);
		fputs("\t", file);
	}
