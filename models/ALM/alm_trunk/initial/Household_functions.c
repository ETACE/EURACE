...
int Household_read_job_vacancies_or_firing_send_applications()
{
	...
	vacancy_array * vacancy_list = init_vacancy_array();
	int i, j, k;
	ON_THE_JOB_SEARCH = 0;
...
/* Create a job offer dynamic array */
	job_offer_array * job_offer_list = init_job_offer_array();
...
int Household_rank_and_buy_goods_1()
{
	...
	logit_firm_id_array * logit_firm_id_list = init_logit_firm_id_array();
	mall_quality_price_info_array * mall_quality_price_info_list = 				init_mall_quality_price_info_array();
