...
/* Things for round-robin partitioning */
	...
	int in_production_liquidity_needs_require_external_financing;
	int in_financial_liquidity_needs_require_external_financing;
	int in_employees;
	int in_wage_offer;
	int in_technology;
	int in_no_employees;
	...
	int in_savings;
	int in_last_income;
	int in_day_of_month_receive_imcome;
	...
	int in_transaction_price;
	int in_transaction_quantity;
	int in_account;
...
//	int production_liquidity_needs_require_external_financing;
//	int financial_liquidity_needs_require_external_financing;
...
//	int day_of_month_receive_imcome;
...
//	double amount_credit_offer;
	...
	in_production_liquidity_needs_require_external_financing = 0;
	in_financial_liquidity_needs_require_external_financing = 0;
	in_employees = 0;
	in_wage_offer = 0;
	in_technology = 0;
	in_no_employees = 0;
	...
	in_savings = 0;
	in_last_income = 0;
	in_day_of_month_receive_imcome = 0;
	...
	in_limit_price = 0;
	in_limit_quantity = 0;
	in_transaction_price = 0;
	in_transaction_quantity = 0;
	in_account = 0;
...
in_Government_agent = 0;
				...
				if(strcmp(agentname, "Firm") == 0)
				{
					...
					if( flag == 0 )
					{
						//add_Firm_agent(id, region_id, share_net_investments, mean_wage, needed_capital_stock, actual_cap_price, mean_specific_skills, planned_production_quantity, production_quantity, unit_costs, production_costs, revenue_per_day, technological_frontier, cum_revenue, out_of_stock_costs, malls_sales_statistics, quality, price, price_last_month, demand_capital_stock, planned_production_costs, adaption_production_volume_due_to_insufficient_finances, fraction_reserved_for_delayed_payments, sold_quantities, total_sold_quantity, cum_total_sold_quantity, delivery_volume, planned_delivery_volume, current_mall_stocks, last_planned_production_quantities, day_of_month_to_act, gov_id, bank_id, ebit, payment_account, total_interest_payment, total_debt_installment_payment, total_dividend_payment, earnings, tax_rate_corporate, tax_payment, net_earnings, previous_net_earnings, loans, total_debt, total_value_local_inventory, capital_stock, total_units_capital_stock, total_value_capital_stock, total_capital_depreciation_value, total_capital_depreciation_units, total_assets, total_liabilities, bankruptcy, production_liquidity_needs, financial_liquidity_needs, total_financial_needs, internal_financial_needs, external_financial_needs, total_external_financing_obtained, production_liquidity_needs_require_external_financing, financial_liquidity_needs_require_external_financing, employees, wage_offer, technology, no_employees, no_employees_skill_1, no_employees_skill_2, no_employees_skill_3, no_employees_skill_4, no_employees_skill_5, vacancies, average_g_skill, average_s_skill_of_1, average_s_skill_of_2, average_s_skill_of_3, average_s_skill_of_4, average_s_skill_of_5, wage_offer_for_skill_1, wage_offer_for_skill_2, wage_offer_for_skill_3, wage_offer_for_skill_4, wage_offer_for_skill_5, employees_needed, current_share_price, current_shares_outstanding, previous_dividend_per_share, current_dividend_per_share, previous_dividend_per_earnings, current_dividend_per_earnings, earnings_per_share_ratio, debt_earnings_ratio, price_earnings_ratio, retained_earnings_ratio, earnings_per_share_ratio_growth, critical_price_earnings_ratio, critical_earnings_per_share_ratio, target_loans, target_equity, bonds_issue_target, stocks_issue_target);
				...
				else if(strcmp(agentname, "Household") == 0)
				{
					...
					if( flag == 0 )
					{
						//add_Household_agent(id, region_id, neighboring_region_ids, budget, week_of_month, weekly_budget, rationed, mall_completely_sold_out, order_quantity, received_quantity, day_of_month_to_act, day_of_week_to_act, day_of_month_receive_income, current_productivity_employer, current_mean_specific_skills_employer, wage, wage_reservation, general_skill, on_the_job_search, number_applications, specific_skill, employee_firm_id, employer_region_id, expenditures, received_dividend_cap, received_dividend_cons, savings, last_income, day_of_month_receive_imcome, household_portfolio);
				...
				else if(strcmp(agentname, "Mall") == 0)
				{
					...
					if( flag == 0 )
					{
						//add_Mall_agent(id, region_id, current_stock, firm_revenues, total_supply);
				...
				else if(strcmp(agentname, "IGFirm") == 0)
				{
					...
					if( flag == 0 )
					{
						//add_IGFirm_agent(id, region_id, productivity, innovation_probability, productivity_progress, capital_good_price, revenue_per_day, day_of_month_to_act);
				...
				else if(strcmp(agentname, "Eurostat") == 0)
				{
					...
					if( flag == 0 )
					{
						...
						add_Eurostat_agent_internal(current_Eurostat_agent, Eurostat_start_Market_Research_state);
				...
				{
					/*posx = current_Clearinghouse_agent->;
					posy = current_Clearinghouse_agent->;
					posz = 0.0;*/
					...
					if( flag == 0 )
					{
						//add_Clearinghouse_agent(id, region_id, trader_id, stock_id, bond_id, gov_bond_id, limit_price, limit_quantity, transaction_price, transaction_quantity);
					...
					else if (flag != 0)
					{
						...
						else if (partition_method == other)
						{
							if (agent_count % number_partitions == 0)
							{
								...
								/*current_Clearinghouse_agent-> = xcentre;
								current_Clearinghouse_agent-> = ycentre;
				...
				else if(strcmp(agentname, "Bank") == 0)
				{
					...
					if( flag == 0 )
					{
						//add_Bank_agent(id, region_id, account, proposed_interest_rate, amount_credit_offer);
...
//				amount_credit_offer = 0.0;
			...
			if(strcmp(buffer, "bankruptcy") == 0) in_bankruptcy = 1;
			if(strcmp(buffer, "/bankruptcy") == 0) in_bankruptcy = 0;
			...
			if(strcmp(buffer, "production_liquidity_needs_require_external_financing") == 0) in_production_liquidity_needs_require_external_financing = 1;
			if(strcmp(buffer, "/production_liquidity_needs_require_external_financing") == 0) in_production_liquidity_needs_require_external_financing = 0;
			if(strcmp(buffer, "financial_liquidity_needs_require_external_financing") == 0) in_financial_liquidity_needs_require_external_financing = 1;
			if(strcmp(buffer, "/financial_liquidity_needs_require_external_financing") == 0) in_financial_liquidity_needs_require_external_financing = 0;
			...
			if(strcmp(buffer, "day_of_month_receive_imcome") == 0) in_day_of_month_receive_imcome = 1;
			if(strcmp(buffer, "/day_of_month_receive_imcome") == 0) in_day_of_month_receive_imcome = 0;
			...
			if(strcmp(buffer, "transaction_price") == 0) in_transaction_price = 1;
			if(strcmp(buffer, "/transaction_price") == 0) in_transaction_price = 0;
			if(strcmp(buffer, "transaction_quantity") == 0) in_transaction_quantity = 1;
			if(strcmp(buffer, "/transaction_quantity") == 0) in_transaction_quantity = 0;
			if(strcmp(buffer, "account") == 0) in_account = 1;
			if(strcmp(buffer, "/account") == 0) in_account = 0;
		...
		else if(c == '<')
		{
			...
			else if(in_Firm_agent == 1)
			{
				...
				if(in_bankruptcy) current_Firm_agent->bankruptcy = atoi(buffer);
				...
				if(in_production_liquidity_needs_require_external_financing) current_Firm_agent->production_liquidity_needs_require_external_financing = atoi(buffer);
				if(in_financial_liquidity_needs_require_external_financing) current_Firm_agent->financial_liquidity_needs_require_external_financing = atoi(buffer);
			...
			else if(in_Household_agent == 1)
			{
				...
				if(in_day_of_month_receive_imcome) current_Household_agent->day_of_month_receive_imcome = atoi(buffer);
			...
			else if(in_Clearinghouse_agent == 1)
			{
				...
				if(in_transaction_price) current_Clearinghouse_agent->transaction_price = atof(buffer);
				if(in_transaction_quantity) current_Clearinghouse_agent->transaction_quantity = atof(buffer);
			}
			else if(in_Bank_agent == 1)
			{
				...
				if(in_account) read_double_static_array(buffer, &j, current_Bank_agent->account, 10);
...
void write_Firm_agent(FILE *file, xmachine_memory_Firm * current)
{
	...
	sprintf(data, "%f", current->share_net_investments);
	fputs(data, file);
	...
	fputs("</total_liabilities>\n", file);
		fputs("<bankruptcy>", file);
	sprintf(data, "%i", current->bankruptcy);
	fputs(data, file);
	fputs("</bankruptcy>\n", file);
	...
	sprintf(data, "%f", current->internal_financial_needs);
	fputs(data, file);
	...
	fputs("</total_external_financing_obtained>\n", file);
		fputs("<production_liquidity_needs_require_external_financing>", file);
	sprintf(data, "%i", current->production_liquidity_needs_require_external_financing);
	fputs(data, file);
	fputs("</production_liquidity_needs_require_external_financing>\n", file);
		fputs("<financial_liquidity_needs_require_external_financing>", file);
	sprintf(data, "%i", current->financial_liquidity_needs_require_external_financing);
	fputs(data, file);
	fputs("</financial_liquidity_needs_require_external_financing>\n", file);
	...
	sprintf(data, "%f", current->earnings_per_share_ratio);
	fputs(data, file);
	...
	sprintf(data, "%f", current->target_loans);
	fputs(data, file);
	fputs("</target_loans>\n", file);
		fputs("<target_equity>", file);
	sprintf(data, "%f", current->target_equity);
	fputs(data, file);
	fputs("</target_equity>\n", file);
		fputs("<bonds_issue_target>", file);
	sprintf(data, "%f", current->bonds_issue_target);
	fputs(data, file);
	fputs("</bonds_issue_target>\n", file);
		fputs("<stocks_issue_target>", file);
	sprintf(data, "%f", current->stocks_issue_target);
	fputs(data, file);
...
void write_Household_agent(FILE *file, xmachine_memory_Household * current)
{
	...
	sprintf(data, "%f", current->received_dividend_cap);
	fputs(data, file);
	fputs("</received_dividend_cap>\n", file);
		fputs("<received_dividend_cons>", file);
	sprintf(data, "%f", current->received_dividend_cons);
	fputs(data, file);
	fputs("</received_dividend_cons>\n", file);
		fputs("<savings>", file);
	sprintf(data, "%f", current->savings);
	fputs(data, file);
	fputs("</savings>\n", file);
		fputs("<last_income>", file);
	write_double_dynamic_array(file, &current->last_income);
	fputs("</last_income>\n", file);
		fputs("<day_of_month_receive_imcome>", file);
	sprintf(data, "%i", current->day_of_month_receive_imcome);
	fputs(data, file);
	fputs("</day_of_month_receive_imcome>\n", file);
...
void write_Clearinghouse_agent(FILE *file, xmachine_memory_Clearinghouse * current)
{
	...
	sprintf(data, "%i", current->trader_id);
	fputs(data, file);
	fputs("</trader_id>\n", file);
		fputs("<stock_id>", file);
	sprintf(data, "%i", current->stock_id);
	fputs(data, file);
	fputs("</stock_id>\n", file);
		fputs("<bond_id>", file);
	sprintf(data, "%i", current->bond_id);
	fputs(data, file);
	fputs("</bond_id>\n", file);
		fputs("<gov_bond_id>", file);
	sprintf(data, "%i", current->gov_bond_id);
	fputs(data, file);
	fputs("</gov_bond_id>\n", file);
		fputs("<limit_price>", file);
	sprintf(data, "%f", current->limit_price);
	fputs(data, file);
	fputs("</limit_price>\n", file);
		fputs("<limit_quantity>", file);
	sprintf(data, "%f", current->limit_quantity);
	fputs(data, file);
	fputs("</limit_quantity>\n", file);
		fputs("<transaction_price>", file);
	sprintf(data, "%f", current->transaction_price);
	fputs(data, file);
	fputs("</transaction_price>\n", file);
		fputs("<transaction_quantity>", file);
	sprintf(data, "%f", current->transaction_quantity);
	fputs(data, file);
	fputs("</transaction_quantity>\n", file);
...
void write_Bank_agent(FILE *file, xmachine_memory_Bank * current)
{
	...
	fputs("</region_id>\n", file);
		fputs("<account>", file);
	write_double_static_array(file, current->account, 10);
	fputs("</account>\n", file);
		fputs("<proposed_interest_rate>", file);
	sprintf(data, "%f", current->proposed_interest_rate);
	fputs(data, file);
