                 ...
                 * volume for one mall by a certain fraction of the last delivery volume*/
                {
                    for(int j = 0; j < PLANNED_DELIVERY_VOLUME.size; j++)
                    {
                        	...
                        	CURRENT_MALL_STOCKS.array[i].mall_id)
                        {
                            ...
                            for(int k=0; k< MALLS_SALES_STATISTICS.size;k++)
                            {
                            	if(MALLS_SALES_STATISTICS.array[k].mall_id==CURRENT_MALL_STOCKS.array[i].mall_id)
                            	{
                            		for(int l=0; l<MALLS_SALES_STATISTICS.array[k].sales.size;l++)
                            		{
                            			MALLS_SALES_STATISTICS.array[k].sales.array[l].sales*=(1+ADAPTION_DELIVERY_VOLUME);
                            		}
                            	}
                            }
...
int Firm_calc_pay_costs()
{
    ...
    /*Calculate the unit costs and total costs*/
        ...
        double mean_production_quantity=0;
               for(int i = 0; i < LAST_PRODUCTION_QUANTITIES.size; i++)
               {
                   mean_production_quantity += LAST_PRODUCTION_QUANTITIES.array[i];
               }
               mean_production_quantity = mean_production_quantity/ LAST_PRODUCTION_QUANTITIES.size;
      ...
      /*      if(PRICE>1.1*PRICE_LAST_MONTH)
            {
            	PRICE=1.1*PRICE_LAST_MONTH;
            }else if(PRICE>0.9*PRICE_LAST_MONTH)
            {
            	PRICE=0.9*PRICE_LAST_MONTH;
            }
           */ 
