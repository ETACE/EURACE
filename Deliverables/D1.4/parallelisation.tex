...
During the design stage of developing \textit{libmboard}, several complications were identified when considering the use of  \textit{pthreads} for implementing the \textit{Communication Thread}. After some consideration, it was decided that the possibilities that the threaded model can provide outweights the drawbacks.

Some of the issues that were identified are:
\begin{itemize}
...
Synchronising handles (boards, iterators, functions). Checking = Blocking operations = large overheads. Use debug v production.
(more...)
...
\subsubsection{Filter tags not widely used}
Filters not widely used (or even fully implemented in FLAME). Full data replication. Slow! See benchmarks.
