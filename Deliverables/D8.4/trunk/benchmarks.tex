...
Using a 16 region model with 26948 agents runs have been carried out on large parallel machines available to STFC, namely HAPU, NW-Grid and Hector. Details of these machines can be found in Appendix \ref{systems}. The graph of average iteration time (seconds) against number of processes is shown in Figure \ref{fig:speedup} from which it can be observed that Hector shows good speedup, NW-Grid shows a little and HAPU shows rather eratic results. The iteration times are given in table \ref{table:speedup}.
