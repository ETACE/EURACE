	...
	while (pos < filebuffer->size)
	{
		...
		if (tmplcode == 1)
		{
			...
			if (c == '>')
			{
				...
				else if (strcmp(buffer->array, "<?if condition?>") == 0)
				{
					...
					{
						if (current_function->condition == NULL) write = 0;
					}
				}
		...
		else
		{
			...
			if (c == '$' && write == 1)
			{
				...
				{
					while (strcmp(buffer3->array, "$name") != 0 && strcmp(buffer3->array, "$agent_name") != 0 && pos <= (pos1 + 15))
					{
						add_char(buffer3, c);
						pos++;
						c = filebuffer->array[pos];
				...
				else if (strcmp("foreach function", lastloop) == 0)
				{
					...
					{
						if(current_function->condition == NULL) fputs("1", file);
						else fputs(current_function->condition, file);
					}
					else
					{
						fputs("$", file);
