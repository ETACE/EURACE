...
void readModel(input_file * inputfile, char * directory, model_data * modeldata, input_file ** p_files)
{
	...
	while(reading == 1 && c != (char)EOF)
	{
		...
		else if(c == '>')
		{
			...
			if(strcmp(current_string->array, "lhs") == 0) { lhs = 1; }
			if(strcmp(current_string->array, "/lhs") == 0) { lhs = 0; }
			if(strcmp(current_string->array, "rhs") == 0) { rhs = 1; }
			if(strcmp(current_string->array, "/rhs") == 0) { rhs = 0; }
			...
			{
				condition = 1;
				//current_rule_data = add_rule_data(&current_function->condition);
			}
			if(strcmp(current_string->array, "/condition") == 0)
			{
				condition = 0;
				//printf("condition: %s %s %s\n", current_rule_data->lhs, current_rule_data->op, current_rule_data->rhs);
			}
		...
		else if(c == '<')
		{
			...
			else if(function)
			{
				...
				if(condition)
				{
					current_function->condition = copy_array_to_str(current_string);
					/* TODO: Need to nest these rules */
					//if(lhs) current_rule_data->lhs = copy_array_to_str(current_string);
		...
		/* If in data read char into buffer */
					...
					|| (state && (name || attribute || (transition && (func || dest))))
						|| (function && (name || note || code || depends || type || cur_state || next_state || input || output || messagetype))
